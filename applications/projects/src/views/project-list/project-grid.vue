...
import { useGridConfig } from "./project-grid-config";
 ...
 * */
 projectStore.$subscribe(() => {
  store.data = projectStore.projects;
})
...
onMounted(() => {
  ...
  gridInstance.on('cellClick', (event) => {
    router.push('/' + route.params.tenantId + '/projects/' + event.record.data.UUID + '/planning');
